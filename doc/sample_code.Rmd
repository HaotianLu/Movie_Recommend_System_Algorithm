...
```{r, eval=FALSE}
#Cross validation takes several hours, so don't run. Instead next code snippets loads from memory
...
```{r, eval= FALSE}
#Parameters for f,lamda, and max iter determined from image above of 5-fold cross validation
...
data_train$bi.pred <- result$bi[1,as.character(data_train$movieId)]
data_train$bu.pred <- result$bu[1,data_train$userId]
...
data_test$bi.pred <- result$bi[1,as.character(data_test$movieId)]
data_test$bu.pred <- result$bu[1,data_test$userId]
...
als.knn.train <- predict(als.knn.model, data.frame(cbind(mu = data_train$mu, bi.pred = data_train$bi.pred, bu.pred = data_train$bu.pred, knn.pred = data_train$knn.pred)))

## get testing prediction
als.knn.test <- predict(als.knn.model, data.frame(cbind(mu = data_test$mu, bi.pred = data_test$bi.pred, bu.pred = data_test$bu.pred, knn.pred = data_test$knn.pred)))
...
#Note train rmse and test rmse are worse than pure ALS with bias. This confirms what the paper claimed
```
